...
import { 
  ArrowLeft, ArrowRight, AlertTriangle, Banknote, Percent, 
  ChevronRight, Filter, ArrowUpDown, GitCompare, 
  ShieldCheck, Activity, Wallet, Sparkles
} from 'lucide-react';
...
const Dashboard = () => {
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-4 relative overflow-hidden">
        ...
        <div className="mt-24 text-center z-10">
          ...
          <p className="text-neutral-400 text-sm">Retrieving your AI eligibility scan.</p>
        </div>
      </div>
    );
  }
  ...
  const selectedForCompare = compareIndices.map((i) => loans[i]).filter(Boolean);
  const goToCompare = () => {
    if (selectedForCompare.length < 2) return;
    navigate('/loan-comparison', {
      state: {
        loans: selectedForCompare.map((loan, i) => ({ ...loan, compareId: compareIndices[i] })),
        inputs,
      },
    });
  };
  ...
  const getStatusStyle = (prob) => {
    if (prob >= 80) return { bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', text: 'text-emerald-400', glow: 'shadow-[0_0_10px_rgba(16,185,129,0.2)]' };
    if (prob >= 50) return { bg: 'bg-yellow-500/10', border: 'border-yellow-500/30', text: 'text-yellow-400', glow: 'shadow-[0_0_10px_rgba(234,179,8,0.2)]' };
    return { bg: 'bg-red-500/10', border: 'border-red-500/30', text: 'text-red-400', glow: 'shadow-[0_0_10px_rgba(239,68,68,0.2)]' };
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-cyan-500/30 relative overflow-hidden">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-20">
        ...
        {/* HEADER SECTION */}
        <div className="mb-10">
          <button onClick={() => navigate('/check-eligibility')} className="flex items-center gap-2 text-neutral-400 hover:text-white transition-colors mb-6 text-sm group">
            <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Edit Profile
          </button>
          ...
          <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
            <div>
              ...
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-2">
                Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">Matches.</span>
              </h1>
              <p className="text-neutral-400 text-lg">
                We found <span className="text-white font-bold">{filteredAndSortedLoans.length}</span> tailored options.
              </p>
            </div>
            ...
            <div className="flex bg-neutral-900/60 backdrop-blur-md border border-white/10 rounded-2xl p-2 shadow-xl">
               <div className="px-6 py-3">
                  <span className="text-neutral-500 text-xs uppercase tracking-widest font-semibold block mb-1">Required Loan</span>
                  <span className="text-2xl font-bold text-white">₹ {(inputs.loanAmount / 100000).toFixed(2)}L</span>
               </div>
               <div className="w-[1px] bg-white/10 my-2"></div>
               <div className="px-6 py-3">
                  <span className="text-neutral-500 text-xs uppercase tracking-widest font-semibold block mb-1">Tenure</span>
                  <span className="text-2xl font-bold text-white">{inputs.tenureYears} Yrs</span>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="sticky top-20 z-30 mb-8 bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-2xl">
          ...
          <div className="flex items-center gap-3 md:border-l border-white/10 md:pl-4">
             ...
             >
               <option value="probability" className="bg-neutral-900">Highest Approval</option>
               <option value="rate" className="bg-neutral-900">Lowest Interest</option>
               <option value="emi" className="bg-neutral-900">Lowest EMI</option>
             </select>
          </div>
        </div>
              ...
              <div className="bg-gradient-to-r from-cyan-900/40 to-purple-900/40 border border-cyan-500/30 rounded-2xl p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4 backdrop-blur-md">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-neutral-400">{selectedForCompare.length} of 4 selected</p>
                  </div>
                </div>
                <button
                  onClick={goToCompare}
        ...
        <div className="grid grid-cols-1 gap-6">
          {filteredAndSortedLoans.length === 0 ? (
            <div className="py-20 text-center bg-neutral-900/20 border border-white/5 rounded-3xl backdrop-blur-sm">
              ...
              <p className="text-neutral-400">Try adjusting your filters to see more options.</p>
            </div>
          ) : (
            <AnimatePresence mode="popLayout">
              {filteredAndSortedLoans.map((loan, index) => {
                ...
                const compareDisabled = compareIndices.length >= 4 && !compareChecked;
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-2xl font-bold text-white tracking-tight">{loan.bankName}</h3>
                          {loan.loanType && (
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold border ${statusStyle.bg} ${statusStyle.border} ${statusStyle.text} ${statusStyle.glow} flex items-center gap-1.5`}>
                    ...
                    <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-center mb-8">
                      ...
                      <div className="flex-1 grid grid-cols-2 gap-4 w-full">
                        <div className="bg-black/40 border border-white/5 rounded-2xl p-4">
                          <div className="flex items-center gap-1.5 text-neutral-400 text-xs uppercase tracking-wider font-semibold mb-2">
                            <Percent size={14} className="text-cyan-400" /> Interest Rate
                          </div>
                          <div className="text-2xl font-bold text-white">{loan.interestRate}%</div>
                        </div>
                        <div className="bg-black/40 border border-white/5 rounded-2xl p-4">
                          <div className="flex items-center gap-1.5 text-neutral-400 text-xs uppercase tracking-wider font-semibold mb-2">
                            <Banknote size={14} className="text-emerald-400" /> Monthly EMI
                          </div>
                          <div className="text-2xl font-bold text-white">₹ {loan.monthlyEMI?.toLocaleString()}</div>
                        </div>
                        <div className="bg-black/40 border border-white/5 rounded-2xl p-4">
                          <div className="flex items-center gap-1.5 text-neutral-400 text-xs uppercase tracking-wider font-semibold mb-2">
                            <Activity size={14} className={loan.foir > 50 ? 'text-red-400' : 'text-purple-400'} /> FOIR Impact
                          </div>
                          <div className="text-xl font-bold text-white">{loan.foir}% <span className="text-sm font-normal text-neutral-500">of income</span></div>
                        </div>
                        <div className="bg-black/40 border border-white/5 rounded-2xl p-4">
                          <div className="flex items-center gap-1.5 text-neutral-400 text-xs uppercase tracking-wider font-semibold mb-2">
                            <Wallet size={14} className="text-yellow-400" /> Total Payable
                          </div>
                          <div className="text-xl font-bold text-white">₹ {(loan.totalAmountPayable / 100000).toFixed(2)}L</div>
                        </div>
                      </div>
                      ...
                      <div className="w-full lg:w-72 bg-black/40 border border-white/5 rounded-2xl p-6 relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-end mb-3 relative z-10">
                          <span className="text-neutral-400 font-medium text-sm">Approval Odds</span>
                          <span className="text-3xl font-bold text-white">
                            {loan.approvalProbability.toFixed(0)}<span className="text-lg text-neutral-500">%</span>
                          </span>
                        </div>
                        ...
                        <p className="text-xs text-neutral-500 mt-3 text-right">Based on AI Risk Engine</p>
                      </div>
                    ...
                    <div className="pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        {loan.features && loan.features.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {loan.features.map((feature, i) => (
                              <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-white/5 text-neutral-300 text-xs border border-white/5">
                                <ShieldCheck size={12} className="text-cyan-500" /> {feature}
                              </span>
                            ))}
                          </div>
                        )}
                        ...
                        <div className="text-sm text-neutral-400 flex items-center gap-2">
                           ...
                           Processing Fee: <span className="text-white font-medium">{loan.processingFee || "Standard"}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-3 w-full md:w-auto">
                        <button
                          onClick={() => setOpenEmi(openEmi === index ? null : index)}
                        ...
                        {loan.link ? (
                          <a
                            ...
                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-8 py-3 rounded-xl bg-white text-black font-bold hover:scale-105 transition-transform text-sm shadow-[0_0_20px_rgba(255,255,255,0.2)]"
                          >
                            Apply Now <ChevronRight size={16} />
                          </a>
                        ) : (
                          <button disabled className="flex-1 md:flex-none px-8 py-3 rounded-xl bg-neutral-800 text-neutral-500 font-bold cursor-not-allowed text-sm">
                            Unavailable
                          </button>
                        )}
                      </div>
